...
const urls = data.tests.map((v) => {
  ...
  if (v.bodyFile) arr.push(v.bodyFile);
  if (v.headers) {
    const headerArr: string[] = [];
    for (const key in v.headers) headerArr.push("--header", `${key}: ${v.headers[key]}`);
...
if (!inTestMode)
  for (const script of data.scripts) {
    const args = [script.type || "bun", `${rootDir}/scripts/${script.file}`] as [string, string];
...
// Run benchmark
{
  ...
  for (let i = 0; i < frameworks.length; ++i) {
    ...
    if (!existsSync(resultDir)) mkdirSync(resultDir);
      ...
      if (info.version === "runtime")
        switch (info.runtime) {
          case "bun":
            ...
            break;
          case "node":
            info.version = Bun.spawnSync(["node", "--version"])
              .stdout.toString()
              .substring(1)
              .replace("\n", "");
            break;
